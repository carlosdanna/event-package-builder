// End-to-end pricing for each template, with exact totals in the smallest currency unit.
...
function lineSummary(lines: ReturnType<typeof buildPackage>) {
  return lines.map((line) => [line.title, line.quantity, line.unitPrice, line.lineTotal]);
}
...
describe("package scenarios", () => {
  it("prices a conference for 45 guests over 2 days, within budget", () => {
    ...
    const lines = buildPackage(template("conference"), basics, testCatalog, "SEK");
  ...
  it("prices the same conference from the euro price list, with no conversion", () => {
    const basics = { guests: 45, startDate: "2026-10-14", endDate: "2026-10-15" };
    const lines = buildPackage(template("conference"), basics, testCatalog, "EUR");

    expect(lineSummary(lines)).toEqual([
      ["Harbour Room", 2, 156_000, 312_000],
      ["Coffee break", 90, 850, 76_500],
      ["Conference lunch", 90, 2_100, 189_000],
      ["Projector and screen", 2, 10_500, 21_000],
    ]);
    expect(summarize(lines, basics)).toMatchObject({ subtotal: 598_500, perPerson: 13_300 });
  });
  ...
  it("prices a wedding for 90 guests on 1 day, with 12 rooms for 1 night", () => {
    ...
    const lines = buildPackage(template("wedding"), basics, testCatalog, "SEK");
  ...
  it("prices a private dinner for 12 guests on 1 day", () => {
    ...
    const lines = buildPackage(template("private-dinner"), basics, testCatalog, "SEK");
  ...
  it("flags a product launch for 180 guests, since no space fits", () => {
    ...
    const lines = buildPackage(template("product-launch"), basics, testCatalog, "SEK");
